...
- [Human Interface Guidelines — Requesting permission](https://developer.apple.com/design/human-interface-guidelines/privacy) — Apple прямо рекомендует объяснять «зачем» **до** системного диалога; наш primer — реализация этой рекомендации.
- [`UNUserNotificationCenter.requestAuthorization(options:)`](https://developer.apple.com/documentation/usernotifications/unusernotificationcenter) — нативный async API для пушей, единственный из permission-API без continuation.
...
- [`AVCaptureDevice.requestAccess(for:)`](https://developer.apple.com/documentation/avfoundation/avcapturedevice) — доступ к камере; completion-based, превращается в async через continuation.
- [`PHPhotoLibrary.requestAuthorization(for:)`](https://developer.apple.com/documentation/photos/phphotolibrary) — доступ к фото с разделением `.readWrite` / `.addOnly` и отдельным состоянием `.limited`.
- [`ATTrackingManager.requestTrackingAuthorization`](https://developer.apple.com/documentation/apptrackingtransparency/attrackingmanager/requesttrackingauthorization(completionhandler:)) — App Tracking Transparency; даже если у тебя нет рекламы, primer перед этим диалогом критичен.

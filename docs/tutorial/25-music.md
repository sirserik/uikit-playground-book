...
mute, играю в фоне». По умолчанию `.playback` НЕ микшируется — при старте
прерывает чужое аудио (Apple Music и т.п.); чтобы играть поверх, нужна
опция `.mixWithOthers`. После этой настройки плеер ведёт себя как Apple
Music или Spotify.

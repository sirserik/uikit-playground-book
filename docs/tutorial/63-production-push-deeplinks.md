...
- [`UNUserNotificationCenter`](https://developer.apple.com/documentation/usernotifications/unusernotificationcenter) — центральная точка работы с локальными и удалёнными уведомлениями.
- [`UNUserNotificationCenterDelegate`](https://developer.apple.com/documentation/usernotifications/unusernotificationcenterdelegate) — `willPresent` для foreground и `didReceive` для tap-обработки.
